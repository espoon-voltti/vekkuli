...
import { MultiSelect, OneOfState } from './form'
    ...
    fn: (prev) => {
      state:
        | StateOf<
            Form<
              ObjectOutput<{
                amenities: MultiSelect<string, never>
                width: Form<number, 'required' | string, string, unknown>
                length: Form<number, 'required' | string, string, unknown>
                harbor: MultiSelect<
                  | { label: 'Haukilahti'; value: '1' }
                  | { label: 'Kivenlahti'; value: '2' }
                  | {
                      label: 'Laajalahti'
                      value: '3'
                    }
                  | { label: 'Otsolahti'; value: '4' }
                  | { label: 'Soukka'; value: '5' }
                  | { label: 'Suomenoja'; value: '6' }
                  | {
                      label: 'Svinö'
                      value: '7'
                    },
                  never
                >
                boatType: Form<string, 'required', OneOfState<string>, unknown>
              }>,
              'required' | string,
              ObjectState<{
                amenities: MultiSelect<string, never>
        ...
        | StateOf<
            ...
            >
          >
      branch: 'Storage' | 'Winter' | 'Slip' | 'Trailer'
    }
  ) => void
