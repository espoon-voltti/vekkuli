const validation = (function () {
  ...
  async function validateField(field) {
    ...
    if (errorElements.required) {
      const isRequired =
        field.hasAttribute("data-required") && !field.hasAttribute("disabled");
      const isEmpty =
        field.type === "checkbox"
          ? !field.checked
    ...
    if (field.hasAttribute("data-validate-url")) {
      try {
        const isServerValid = await validateFieldWithServer(
          field,
          errorElements.server,
        );
        updateGlobalErrorMessage(field.closest("form"));
  ...
  function setupFormValidation(form) {
    ...
    form.addEventListener(
      ...
      async function (event) {
        if (
          event.target.matches('button[type="submit"], input[type="submit"]')
        ) {
    ...
    form.addEventListener("input", function (event) {
      ...
      if (
        field.matches("[data-required], [data-pattern], [data-validate-url]")
      ) {
        validateField(field);
      }
    });
